...
contract killerGF is ERC721URIStorage,Ownable{
    ...
    /**
      * @dev 批量铸造NFT
      * @param tokenURIs_ NFT显示图片链接
      */
    function batchMint(string[] memory tokenURIs_) public onlyOwner returns (uint256[] memory){
        
        uint256[] memory tokenIds = new uint256[](BATCH_NUM);
        for(uint8 i = 0 ; i < tokenURIs_.length ; i++){
            uint256 tokenId = mint(tokenURIs_[i]);
            tokenIds[i] = tokenId;
        }
        return tokenIds;
    }
